 ...
 *			sync transaction was rejected
 *		BR_TRANSACTION_PENDING_FROZEN if the target process is
 *			frozen and the async transaction was successfully queued
 */
				 ...
				 struct binder_handle_cookie *handle_cookie)
{
	...
	ref->freeze = NULL;
	/*
	 * Reuse the existing freeze object so that in-flight notifications can
	 * either be replaced in-place or resent after userspace acknowledges the
	 * currently delivered one.
	 */
